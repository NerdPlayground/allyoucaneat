...
def get_error_message(data):
    error_response= str()
    for error_key,error_value in data:
...
def payment_process_results(request):
    # http://127.0.0.1:8000/orders/payment-process-results/
    data= request.data
...
def pay_order(request,pk):
    ...
    payload={
        ...
        "TransactionDesc": "Pay for %s" %(order.product),
    ...
    if response.status_code == 200:
        ...
        return HttpResponseRedirect(reverse("orders:complete-payment",args=(order.id,)))
    else:
...
def complete_payment(request,pk):
    ...
    if request.method == "POST":
        request_payment_response= RequestPaymentResponse.objects.filter(
